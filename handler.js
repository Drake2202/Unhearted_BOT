...
export async function participantsUpdate({ id, participants, action }) {
                ...
                for (let user of participants) {
                    ...
                    let about = (await this.fetchStatus(user).catch(console.error) || {}).status || '×'
                        ...
                        [(action == 'add' ? 'Gracias ❤' : 'Adios 👋'), 'ura'],    
                            ['Menú 📒', '/menu']
                            ], fgclink, { contextInfo: { externalAdReply: { showAdAttribution: false, title: '𝐶𝑅𝐼𝑊𝐼𝐿𝑂𝑃⚔𝐵𝑂𝑇࿐ Sᴜᴘᴘᴏʀᴛ❐🎋༉', body: global.botname, sourceUrl: global.linkgc, thumbnail: miniurl }}, mentions: [user]})
                    }
                }
            }
            break
  ...
  let bio = biot.status?.toString() || 'Sin Info'
            if (chat.detect)
                this.sendButton(id, text, saludo, pp, [['Menú 📒', '/menu']], false, { contextInfo: { externalAdReply: { showAdAttribution: false, title: '𝐶𝑅𝐼𝑊𝐼𝐿𝑂𝑃⚔𝐵𝑂𝑇࿐ Sᴜᴘᴘᴏʀᴛ❐🎋༉', body: global.botname, sourceUrl: global.linkgc, thumbnail: miniurl }}, mentions: this.parseMention(text) })
            break
            }
            }
            }
...
let date = d.toLocaleDateString('es', { day: 'numeric', month: 'long', year: 'numeric' })
 let time = d.toLocaleString('en-US', { hour: 'numeric', minute: 'numeric', second: 'numeric', hour12: true })
    try {
        ...
        if (chat.delete)
            return
        await conn.relayMessage(msg.chat, { extendedTextMessage: { text: `┏━━━━⬣  𝘼𝙉𝙏𝙄 𝘿𝙀𝙇𝙀𝙏𝙀  ⬣━━━━⬣\n┃🐢 *❏ Nombre:* @${participant.split`@`[0]}\n┃🕒 *❏ Hora:* ${time}\n┃📅 *❏ Fecha:* ${date}\n┃🥗 *❏ Enviando mensaje...*\n┗━━━━━━━━━━━━━━━━━⬣`, contextInfo: { externalAdReply: { title: '𝐶𝑅𝐼𝑊𝐼𝐿𝑂𝑃⚔𝐵𝑂𝑇࿐ Sᴜᴘᴘᴏʀᴛ❐🎋༉', body: '', thumbnail: verificado, sourceUrl: linkgc }, mentionedJid: [participant] }}}, { quoted: fakemsg })
...
global.dfail = (type, m, conn) => {
    ...
    let prep = generateWAMessageFromContent(m.chat, { extendedTextMessage: { text: msg, contextInfo: { externalAdReply: { title: '𝐶𝑅𝐼𝑊𝐼𝐿𝑂𝑃⚔𝐵𝑂𝑇࿐', body: '', thumbnail: miniurl, sourceUrl: script }}}}, { quoted: m })
